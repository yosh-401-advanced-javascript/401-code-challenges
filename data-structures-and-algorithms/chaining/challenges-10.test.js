...
const totalSum = (input) => input.reduce((acc, val) => {
  return acc.concat(val);
}, []).reduce((acc, val) => {
  console.log(acc);
  return acc = acc + val;
...
const divisibleByFiveTwoToThePower = (input) => input.map((elephant => {
  return elephant.filter(num => (num % 5 === 0) && Number.isInteger(num) === true).map(num => 2 ** num)
}));
...
let findMaleAndFemale = (data) => data.filter(elephant => elephant.gender === 'male' || elephant.gender === 'female').reduce((acc, val) => {
