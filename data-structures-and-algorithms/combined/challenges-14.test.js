...
const toTitleCase = arr => arr.map(elephant => elephant.charAt(0).toUpperCase() + elephant.slice(1));
...
let biggerThanLuke = arr => {
  let plusSize = arr.filter(person =>{
      if(parseInt(person.mass) > 77){
          return person;
      }
  });
  let named = plusSize.reduce((acc, val) => {
      acc = `${acc}${val.name} - `;
      return acc;
  }, '');
...
describe("Testing challenge 3", () => {
  test("It should sort items by a price", () => {
    expect(sortBy("price", [{ name: "Sweatshirt", price: 45 }, { name: "Bookmark", price: 2.5 }, { name: "Tote bag", price: 15 }])).toStrictEqual([
  ...
  test("It should sort items by name", () => {
    expect(sortBy("name", [{ name: "Sweatshirt", price: 45 }, { name: "Bookmark", price: 2.5 }, { name: "Tote bag", price: 15 }])).toStrictEqual([
      { name: "Bookmark", price: 2.5 },
      { name: "Sweatshirt", price: 45 },
      { name: "Tote bag", price: 15 }
    ]);
