...
const oddValues = (arr) => arr.filter(nums => (nums % 2));
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
const filterStringsWithVowels = (arr) => arr.filter(vowels => vowels.match(/[aeiou]/gi));
  // Solution code here...
...
const notInFirstArray = (forbiddenValues, arr) => arr.filter(elephant => forbiddenValues.indexOf(elephant) < 0);
...
const getBaseStatGreaterThan = (arr, minBaseStat) => arr.filter( elephant => elephant.baseStat > minBaseStat);
...
const getStatName = (arr, minBaseStat) =>{
  let baseStats = arr.filter(elephant => {
    if (elephant.baseStat > minBaseStat){
      return elephant;
    }
  })
  let nameArray = [];
  baseStats.forEach(elephant => {
    nameArray.push(elephant.stat.name);
  })
  return nameArray;
};
...
const getCharactersWithoutChildren = (arr) => arr.filter(elephant => !elephant.children);
  // Solution code here...
...
const evenOddNumericValues = (arr) => {
  let numbers = arr.filter( elephant => (Number.isInteger(elephant)));
