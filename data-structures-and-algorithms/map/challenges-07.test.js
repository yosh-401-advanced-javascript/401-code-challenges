...
const forEachTwoToThe = (arr) => {
    let newArr = [];
    arr.forEach(elephant => {
        newArr.push(2 ** elephant);
});
return newArr;
...
const charCode = (arr) => arr.map(elephant => elephant.charCodeAt());
...
const evenOdd = (arr) => arr.map(elephant => elephant %2 === 0 ? "even" : elephant %2 > 0 ? "odd" : "N/A");
...
const extractAbilities = (arr) => arr.map(elephant =>  elephant.ability.name);
  // Solution code here...
...
const extractStats = (arr) => arr.map(elephant => ({ name: elephant.stat.name, total: elephant.effort + elephant.baseStat}));
  // Solution code here...
