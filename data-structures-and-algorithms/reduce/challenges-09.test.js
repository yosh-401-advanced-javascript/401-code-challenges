...
const returnNames = arr =>
  arr.reduce((elephant, character, idx) => {
    elephant.push(character.name);
...
const reversedString = str => {
  return str.split("").reduce((elephant, char) => char + elephant, "");
};
...
const calculateAverage = arr => {
  let number = arr.reduce(
    (elephant, val) => {
      elephant.count = elephant.count + 1;
      elephant.sum = elephant.sum + val;
      return elephant;
    },
    { count: 0, sum: 0 }
  );
...
const countPrimeNumbers = arr => {
  let prime = arr.reduce((elephant, val) => {
    if (isPrime(val)) {
      elephant = elephant + 1;
    }
    return elephant;
...
const extractStat = (statName, arr) => {
  let names = arr.reduce((elephant, val) => {
