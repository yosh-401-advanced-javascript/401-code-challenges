...
const listFoods = recipe => {
  let result = [];
  gruffaloCrumble.ingredients.forEach(thing => {
    let string1 = thing.slice(thing.indexOf(" ") + 1);
...
const stepActions = recipe => {
  let result = [];
  gruffaloCrumble.steps.forEach(thing => {
    let step1 = thing.split(" ");
    result.push(step1[0]);
  });
  // Solution code here...
  return result;
